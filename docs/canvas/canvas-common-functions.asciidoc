...
TODO: add a flag for multi args?
...
Returns `true` if all of the conditions are `true`. See also <<any_fn>>.
...
Converts between core types, for example, `string`, `number`, `null`, `boolean`, `date` and renames columns. See also <<mapColumn_fn>> and <<staticColumn_fn>>.
...
Returns `true` if any of the conditions are `true`. See also <<all_fn>>.
...
Configures axis of a visualization, only used in <<plot_fn>>
...
|`min`
|`number`, `string`
|Minimum value displayed in the axis. Must be a `number`, a date in milliseconds since epoch, or an ISO8601 `string`

|`max`
|`number`, `string`
|Maximum value displayed in the axis. Must be a `number`, a date in milliseconds since epoch, or an ISO8601 `string`

|`tickSize`
|`number`
|Increment size between each tick. Use for `number` axes only.
|===
...
Usually used in combination with <<if_fn>>. This only works with primitive types, 
...
|`backgroundSize`
|`string`
|Valid CSS background size string 

Default: `"contain"`

|`backgroundRepeat`
|`string`
|Valid CSS background repeat string 
...
|`valueColumn` ***
|`string`
|The column or field from which to extract the unique values 
for the dropdown

|`filterGroup`
|`string`
|Group name for the filter
|===
...
Alias: `format`
|`string`
|MomentJS Format with which to bucket, see the https://momentjs.com/docs/#/displaying/[MomentJS documentation]
|===
...
Turns a `number` into a `string` using a NumeralJS format.
...
Alias: `format`
|`string`
|NumeralJS format string, see the http://numeraljs.com/#format[NumeralJS documentation]
|===
...
Aliases: `column`, `c`
|`string`
|The name of the column value to fetch, defaults to the first column
...
*Returns:* Depends on your input and arguments (should be only valid column types, `string`, `boolean`, `number`, `null`?)
...
*Accepts:* `any` (should this be `boolean`, `number`, `string`? maybe just `number` and `string`)
...
Gets the first N rows from the `datatable`. Also see `tail`(insert link to tail function?).
...
Alias: `count`
|`number`
|Return this number of rows from the beginning of the `datatable` 
...
Alias: `condition`
|`boolean`
|A `true` or `false`, usually returned by a subexpression. 
...
|`palette`
|`palette`
|A palette object for describing the colors to use on this pie 
...
Set to `"auto"` to automatically set radius 

Default: `"auto"`)

|`hole`
|`number`
|Draw a hole in the pie, 0-100, as a percentage of the pie radius 
...
|`legend`
|`string`, `boolean`
|The legend position, `"nw"`, `"sw"`, `"ne"`, `"se"`, or `false` 
...
|`palette`
|`palette`
|A `palette` object for describing the colors to use on this plot 

Default: `{<<palette_fn>>}`

|`font`
|`style`
|The legend and tick mark fonts 
...
|`yaxis`
|`boolean`, `axisConfig`
|Axis configuration, or use `false` to disable 
...
specialized function instead, such as `<<plot_fn>>`.

|`css`
|`string`
|Any block of custom CSS to be scoped to this element 
...
|`max`
|`number`
|Maximum number of times the image may repeat 
...
|`image`
|`string`
|The image to reveal. An image asset can be provided as a base64 dataurl, or passed in as a subexpression 
(see example below) 
...
|`origin`
|`string`
|Where to start from, eg `top`, `left`, `bottom`, or `right` 
...
Rounds milliseconds since epoch using a MomentJS formatting `string` and returns 
...
Alias: `format`
|`string`
|MomentJS Format with which to bucket. See the 
...
Returns the number of rows. Pairs with `<<ply_fn>>` to get the count of unique column 
...
|`fill`
|`number`, `boolean`
|Designates if points should be filled 
...
|`horizontalBars`
|`boolean`
|Sets the orientation of bars in the chart to horizontal 
...
|`maintainAspect`
|`boolean`
|Select `true` to maintain aspect ratio 
...
Performs conditional logic with multiple conditions. See also <<case_fn>>,
which builds a `case` to pass to the <<switch_fn>> function.
...
Alias: `case`
|`case`
|The list of conditions to check. See <<case_fn>>

|`default` 

Alias: `finally`
|`any`
|The default returned value, if no cases match
|===
...
|`paginate`
|`boolean`
|Show pagination controls. If set to `false` only the first 
page is displayed.  
...
Gets the last N rows from the end of a `datatable`. See also <<head_fn>>
...
|`compact`
|`boolean`
|Show the time filter as a button that triggers a popover 
...
Alias: `type`
|`string`
|A known type, See (TODO: link to canvas types here).
|===
