...
NOTE: The report Generation URL may contain date-math expressions that cause the watch to fail with a `parse_exception`. Ensure that the `{` `}` curly brackets are removed from around the date math expression in the URL, and all required characters in the expression are properly URL-encoded.  The following example snippet from a watch Generation URL uses properly formatted date-math to filter for documents from the last 15 minutes : 
`(range:(%27@timestamp%27:(gte:now-15m%2Fd,lte:now%2Fd))))`
